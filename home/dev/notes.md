...
Two tools can be used interchangeably: `xxd` and `od`. Examples

    $ printf '\x0a\x02\x48\x69' | xxd -p
	0a024869
	$ printf '\x0a\x02\x48\x69' | od -v -tx1 -A n | tr -d ' \n'
	0a024869

Without the flags we get formatted output with offset addresses.

The inverse (hex to bytes) is best done with `xxd`:

    $ printf '0a024869' | xxd -r -p | xxd
    00000000: 0a02 4869                                ..Hi
